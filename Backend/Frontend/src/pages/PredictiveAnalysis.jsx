...
// Forecastable metrics come from the dataset itself (dashboard.metrics is
// the list of numeric columns InsightIQ detected) — there's no fixed set
// like "Revenue/Orders" since the uploaded data could be any domain.
...
export default function PredictiveAnalysis() {
  ...
  useEffect(() => {
    ...
    if (!forecast && dashboard.primaryMetric) load(dashboard.primaryMetric)
  ...
  const load = async (m) => {
    ...
    try {
      ...
      setError(err.message || 'Could not generate a forecast for this metric.')
      setForecast(null)
    } finally {
      setLoading(false)
    }
    ...
    <div className="min-h-screen bg-paper text-ink">
      ...
      <section className="mx-auto max-w-6xl px-6 py-10">
        ...
        <div className="ledger-rule mt-4 bg-white/60 p-5 pt-6">
          {error ? (
            <div className="flex h-72 flex-col items-center justify-center gap-1 text-center font-mono text-xs text-slate">
              <span className="text-alert">Forecast unavailable</span>
