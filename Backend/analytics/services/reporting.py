...
def build_report_pages(dataset, dashboard_data, forecast_data):
    ...
    # --- Data Overview & Methodology (adds formality/transparency) ---
    issues = dataset.issues or []
    issue_lines = [i['label'] for i in issues] if issues else ['No data-quality issues were flagged.']
    date_col = dashboard_data.get('dateCol')
    methodology_bits = [
        f"This report is generated from {dataset.rows:,} rows and {dataset.columns} columns "
        f"({', '.join(m['name'] for m in dataset.file_meta) if dataset.file_meta else 'uploaded data'}), "
        f"with an overall data-quality score of {dataset.quality_score}/100.",
        'Data preparation: ' + '; '.join(issue_lines) + '.',
    ]
    if forecast_data:
        methodology_bits.append(
            f"Forecasting method: {forecast_data['model']}, backtested against held-out historical "
            f"periods (MAPE {forecast_data['accuracy']['mape']}%, RMSE {forecast_data['accuracy']['rmse']}). "
            f"Confidence bands widen with distance from the last observed period ({date_col or 'time axis'})."
        )
    else:
        methodology_bits.append(
            'No forecast was generated for this report - see the Forecast & Outlook section for why.'
        )
    methodology_body = ' '.join(methodology_bits)
    ...
    return [
        ...
        {'title': 'Data Overview & Methodology', 'body': methodology_body},
